...
subsetPM25SSC <- subset(PM25SSC, (PM25SSC$fips=="24510"| PM25SSC$fips=="06037"))
subsetPM25SSC[subsetPM25SSC$fips=="24510",]$fips <-"Baltimore"
subsetPM25SSC[subsetPM25SSC$fips=="06037",]$fips <-"Los Angeles"

#subsetPM25SSC[subsetPM25SSC$fips=="06037",subsetPM25SSC$City] <- "LOS"


## use Data.Category == "Onroad" for motor vehicle source
motorPM25SSC <- subset(subsetPM25SSC, subsetPM25SSC$Data.Category=="Onroad",select=c(SCC,fips,Emissions,year))
...
g3 <- ggplot(motorMeanByYearFips, aes(year, log10(Emissions))) +
      ...
      ggtitle("PM25 Emission Mean by Year")
